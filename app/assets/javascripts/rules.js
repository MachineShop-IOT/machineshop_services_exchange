...
function CreateAllRuleConditionSelect(select_id) {
  ...
  return CreateConditionSelect(combined_array, select_id, "rule_condition_type input-block-level custom-select");
}

function CreateComparisonRuleConditionSelect(select_id) {
  return CreateConditionSelect(COMPARISON_CONDITION_TYPES, select_id, "comparison_rule_condition_type input-block-level custom-select").prepend("<option value=''>please select one...</option>");
}
...
function CreateRuleConditionDiv(condition_type, device_id){
  ...
  case 'less_than_equal_rule_condition':
    ...
    //create an attribute textbox
    html = $(html).append(CreateSimpleComparisonTextBox(id + "_property", "property input-block-level").attr('placeholder', 'Enter payload, e.g., latitude, longitude, etc.'));

    //add the operator
    html = $(html).append(Line(operator));

    //create a textbox for the comparison value
    html = $(html).append(CreateSimpleComparisonTextBox(id + "_value", "value input-block-level"));
    break;
  ...
  case 'not_in_rule_condition':
    ...
    html = $(html).append(CreateSimpleComparisonTextBox(id + "_value_array", "value_array input-block-level"));
    break;
  ...
  case 'not_near_rule_condition':
    ...
    html = $(html).append(Line("Latitude payload property:"));
    html = $(html).append(CreateSimpleComparisonTextBox(id + "_lat_property", "lat_property input-block-level")); //.val("latitude").prop('disabled', true));
    html = $(html).append(Line("Longitude payload property:"));
    html = $(html).append(CreateSimpleComparisonTextBox(id + "_long_property", "lng_property input-block-level")); //.val("longitude").prop('disabled', true));
    //create a textbox for radius
    html = $(html).append(Line(operator));
    html = $(html).append(CreateSimpleComparisonTextBox(id + "_comp_radius", "comp_radius input-block-level"));

    //create 2 textboxes for reference point
    html = $(html).append(Line("of this latitude:"));
    html = $(html).append(CreateSimpleComparisonTextBox(id + "_comp_lat", "comp_lat input-block-level"));
    html = $(html).append(Line("and this longitude:"));
    html = $(html).append(CreateSimpleComparisonTextBox(id + "_comp_long", "comp_long input-block-level"));

    //create a select for units
    html = $(html).append(Line("using these units:"));
    html = $(html).append("<select id='"+id+"_comp_unit' class = 'comp_unit custom-select''><option value = 'ft'>Feet</option><option value = 'mi'>Miles</option><option value = 'm'>Meters</option><option value = 'km'>Kilometers</option></select>")
    break;
  }
  html = $(html).append(Line("Dependency on this parent or sibling payload property:"));
  html = $(html).append(CreateSimpleComparisonTextBox(id + "_pos_property", "pos_property input-block-level"));
  html = $(html).append(Line("with a value of"));
  html = $(html).append(CreateSimpleComparisonTextBox(id + "_pos_value", "pos_value input-block-level"));
  return html;
}
...
function ActionPanel(then_else){
  ...
  html = $(html).append(CreateActionSelect(then_else + "_action_type_" + action_counter, then_else + "_type input-block-level custom-select"));
  html = $(html).append(CreateSimpleComparisonTextBox(then_else + "_action_send_to_" + action_counter, then_else + "_send_to input-block-level"));
